...
public class VendorUrlRepository : Repository<VendorUrl>
{
    ...
    /// <summary>
    /// Atomically attempts to set the URL row's <c>CrawlInProgress</c> flag to true.
    /// Returns true if the flag was claimed (was previously false); returns false if
    /// another crawl already holds the lock or the URL does not exist. Use to prevent
    /// two concurrent crawls from clobbering the same URL row.
    /// </summary>
    public bool TryMarkCrawlInProgress(string urlId)
    {
        if (string.IsNullOrWhiteSpace(urlId)) return false;
        var rowsAffected = conn.Execute(
            "UPDATE vendor_urls SET CrawlInProgress = TRUE WHERE Id = @urlId AND CrawlInProgress = FALSE",
            new { urlId });
        return rowsAffected == 1;
    }

    /// <summary>
    /// Unconditionally clears the URL row's <c>CrawlInProgress</c> flag. Idempotent.
    /// </summary>
    public void ClearCrawlInProgress(string urlId)
    {
        if (string.IsNullOrWhiteSpace(urlId)) return;
        conn.Execute(
            "UPDATE vendor_urls SET CrawlInProgress = FALSE WHERE Id = @urlId",
            new { urlId });
    }
