...
function main() {
  ...
  register(
    "mcp_partners_list",
    "List partner directories (e.g. Xerces) and their ids, for use with set_vendor_partner.",
    {},
    async () => {
      const list = await api.apiGet<unknown[]>("/Vendor/PartnersApi", {});
      return textResult(Array.isArray(list) ? list : []);
    }
  );
  ...
  register(
    "set_vendor_partner",
    "Set or clear a vendor's partner-directory link (e.g. Xerces). Pass partner='' (or omit) to clear the link. externalKey is the partner-side id for this vendor (optional). Use mcp_partners_list for valid partner ids.",
    { vendorId: z.string(), partner: z.string().optional(), externalKey: z.string().optional() },
    async ({ vendorId, partner, externalKey }) => {
      const res = await api.apiPost<unknown>("/Vendor/SetPartnerApi", {
        VendorId: vendorId,
